...
public class FileDownloadController {
	...
	public ModelAndView fileDownloadList(ModelAndView mv) {
		File path = new File("C:/Users/gjwns/Documents/springworkspace/upload");
	...
	public void fileDownload(@PathVariable String file, HttpServletResponse response) throws IOException {
		...
		File f = new File("C:/Users/gjwns/Documents/springworkspace/upload", file);
		
		// 파일명 인코딩
		String encodingFileName = new String (file.getBytes("UTF-8"), "ISO-8859-1");
		// file 다운로드 설정
		response.setContentType("application/download");
		response.setContentLength((int)f.length());
		response.setHeader("Content-Disposition", "attachment;filename=\"" + encodingFileName + "\"");
		// 다운로드 시 저장되는 이름은 Response Header의 "Content-Dispostion"에 위치
